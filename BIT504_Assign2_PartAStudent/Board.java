...
public class Board {
    ...
    public boolean isDraw() {
        ...
        // Hint: Use a nested loop to iterate through cells and check for empty ones.
        for (int row = 0; row < GameMain.ROWS; ++row) {
            for (int col = 0; col < GameMain.COLS; ++col) {
    ...
    public boolean hasWon(Player thePlayer, int playerRow, int playerCol) {
        ...
        // Check for 3 in the diagonal (top-left to bottom-right)
        if (playerRow == playerCol && cells[0][0].content == thePlayer && cells[1][1].content == thePlayer
                && cells[2][2].content == thePlayer) {
            return true;
        }

        // Check for 3 in the other diagonal (top-right to bottom-left)
